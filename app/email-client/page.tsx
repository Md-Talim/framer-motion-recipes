...
export default function EmailClientPage() {
  ...
  const toggleMessage = (mid: number) => {
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      const isControlPressed = event.ctrlKey || event.metaKey; // For Mac support, metaKey is used
      const isAPressed = event.key === "a" || event.key === "A";
      const isEscaped = event.key === "Escape";
      const isDeletePressed = event.key === "Delete";
